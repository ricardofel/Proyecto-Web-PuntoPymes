...
def webhook_nuevo_permiso(sender, instance, created, **kwargs):
    if created:
        hooks = Webhook.objects.filter(evento=EventosWebhook.SOLICITUD_APROBADA, activo=True)
        payload = {
...
def webhook_check_in(sender, instance, created, **kwargs):
    # Ejemplo: Si quisieras disparar un webhook genérico de asistencia
    # Por ahora lo dejamos comentado o implementamos si tienes el evento en constants
    pass