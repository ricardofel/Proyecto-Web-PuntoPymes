...
class IntegracionErp(models.Model):
    ...
    nombre = models.CharField(
        max_length=50,
        help_text="Ej: SAP HANA, Oracle Netsuite"
    )
    url_api = models.URLField(
        help_text="Endpoint principal del sistema externo"
    )
...
class Webhook(models.Model):
    """
    Configuración de Webhooks salientes por evento del sistema.
    """
    EVENTOS = EventosWebhook.OPCIONES

    nombre = models.CharField(
        max_length=100,
        help_text="Ej: Notificar a Slack RRHH"
    )
    ...
    url_destino = models.URLField(
        help_text="URL donde se enviará el POST"
    )
    secret_key = models.CharField(
        max_length=100,
        blank=True,
        null=True,
        help_text="Para firmar el payload"
    )
    activo = models.BooleanField(default=True)
...
class LogIntegracion(models.Model):
    ...
    codigo_respuesta = models.IntegerField(
        help_text="Ej: 200, 404, 500"
    )
    mensaje_respuesta = models.TextField(blank=True)

    def __str__(self):
        return f"{self.fecha:%Y-%m-%d %H:%M} - {self.codigo_respuesta}"
